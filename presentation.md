...
```mermaid
flowchart LR
    subgraph ClientLayer["입력 계층"]
        CLI["CLI\n.sql 파일 / REPL"]
        HTTP["외부 클라이언트\nHTTP POST /query"]
    end

    subgraph ServerLayer["서버/연결 계층"]
        API["API Server\nsocket + router"]
        FACADE["DB Engine Facade\nSQL 실행 진입점"]
    end

    subgraph EngineLayer["내부 DB 엔진"]
        TOK["Tokenizer\nSQL -> Token"]
        PARSER["Parser\nToken -> SqlStatement"]
        EXEC["Executor\nINSERT / SELECT 실행"]
    end
    ...
    subgraph OutputLayer["출력 계층"]
        CLI_OUT["터미널 표 출력"]
        JSON_OUT["JSON HTTP 응답"]
    end

    CLI --> FACADE
    HTTP --> API --> FACADE
    FACADE --> TOK --> PARSER --> EXEC
    EXEC --> TABLE
    TABLE --> BPTREE
    TABLE --> CSV
    EXEC --> FACADE
...
> 이 프로젝트에는 CLI와 API 서버라는 두 개의 진입점이 있습니다. CLI는 터미널에서 직접 SQL을 실행하고, API 서버는 HTTP 요청으로 SQL을 받습니다. 하지만 SQL이 들어간 뒤에는 둘 다 `db_engine_facade`를 통해 같은 DB 엔진 흐름을 사용합니다.
